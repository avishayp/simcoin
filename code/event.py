        ...
        try:
            ...
            with open(config.ticks_csv, 'r') as file:
                ...
                for i, line in enumerate(file):
                    ...
                    planned_start_next_tick = start_time + \
                        (i + 1) * self._context.args.tick_duration
                    ...
                    if current_time < planned_start_next_tick:
                        ...
                        logging.info(
                            'Sleep {} seconds for next tick={}'.format(
                                difference, i))
                        utils.sleep(difference)
        except Exception:
            logging.exception(
                'Simulation could not execute all events because of an exception')
    ...
    def _execute_cmd(self, cmd):
        ...
        if cmd_parts[0] == 'tx':
            ...
            try:
                ...
                logging.exception(
                    'Could not generate tx for node={}'.format(
                        node.name))
            self._txs_count += 1
            ...
            try:
                ...
                logging.exception(
                    'Could not generate block for node={}'.format(
                        node.name))
            ...
            raise SimulationException(
                'Unknown cmd={} in {}-file'.format(cmd_parts[0], config.ticks_csv))
